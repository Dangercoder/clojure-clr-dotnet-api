(ns clj-api.integrant.core
   "Copy of https://github.com/weavejester/integrant with any keys parameters named ks to 
    prevent name clashing."
  (:refer-clojure :exclude [ref read-string run!])
  (:require #?(:clj  [clojure.edn :as edn]
               :cljr  [clojure.edn :as edn]
               :cljs [clj-api.tools.reader.edn :as edn])
            [clojure.walk :as walk]
            [clojure.set :as set]
            [clojure.spec.alpha :as s]
            [clojure.string :as str]
            [clj-api.integrant.dependency :as dep]))
...
(defn- composite-key? [ks]
  (and (vector? ks) (every? qualified-keyword? ks)))
...
(defn- find-keys [config ks f]
  (let [graph  (dependency-graph config {:include-refsets? false})
        keyset (set (mapcat #(map key (find-derived config %)) ks))]
    (->> (f graph keyset)
         (set/union keyset)
         (sort (key-comparator (dependency-graph config))))))

(defn- dependent-keys [config ks]
  (find-keys config ks dep/transitive-dependencies-set))

(defn- reverse-dependent-keys [config ks]
  (reverse (find-keys config ks dep/transitive-dependents-set)))
...
#?(:clj
   (defn load-namespaces
     ...
     ([config ks]
      (doall (->> (dependent-keys config ks)
...
(defn- run-loop [system ks f]
  (loop [completed (), remaining ks]
    (when (seq remaining)
...
(defn run!
  ...
  (run-loop system (dependent-keys (system-origin system) ks) f))
...
(defn reverse-run!
  ...
  [system ks f]
  {:pre [(map? system) (some-> system meta ::origin)]}
  (run-loop system (reverse-dependent-keys (system-origin system) ks) f))
...
(defn build
  ...
  ([config ks f]
   (build config ks f (fn [_ _ _])))
  ([config ks f assertf]
   (build config ks f assertf (fn [_ v] v)))
  ([config ks f assertf resolvef]
   {:pre [(map? config)]}
   (def config config) 
   (def ks ks)
   (let [relevant-keys   (dependent-keys config ks)
...
(defn init
  ...
  ([config ks] 
   {:pre [(map? config)]}
   (build config ks init-key assert-pre-init-spec resolve-key)))
...
(defn halt!
  ...
  ([system ks]
   {:pre [(map? system) (some-> system meta ::origin)]}
   (reverse-run! system ks halt-key!)))

(defn- missing-keys [system ks]
  (remove (set ks) (keys system)))

(defn- halt-missing-keys! [config system ks]
  (let [graph        (-> system meta ::origin dependency-graph)
        missing-keys (missing-keys system (dependent-keys config ks))]
...
(defn resume
  ...
  ([config system ks]
   ...
   (halt-missing-keys! config system ks)
   (build config ks 
          (fn [k v]
...
(defn suspend!
  ...
  ([system ks]
   {:pre [(map? system) (some-> system meta ::origin)]}
   (reverse-run! system ks suspend-key!)))
